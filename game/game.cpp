...
//			test_drawer.draw({ Bheem.body.x, Bheem.body.y },
...
//			test_drawer.draw({ Duryodhan.body.x , Duryodhan.body.y },
...
{
	state_manager state;
	state.initialize(new main_menu);
	state.pre_loop_functions();
	while (!state.renderer->check_window_status())
	{
